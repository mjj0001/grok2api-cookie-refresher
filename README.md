...
| **5** | **UA 指纹检查** | **防 403 神器**。自动对比本地浏览器与服务器的 User-Agent，支持一键同步更新。 |
...
**Q: 怎么查看我的浏览器 User-Agent (UA)？**
A: 
1. 在 Chrome 地址栏输入 `chrome://version/` 并回车。
2. 找到 **"用户代理" (User Agent)** 这一行，后面那一长串就是你的 UA。
3. 如果你需要手动配置 `config.toml`，请确保里面的 `user_agent` 字段与这里查到的一致。
